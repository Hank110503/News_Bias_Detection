...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Qwen2.5-VL Stage 1.")
...
def resolve_resume_checkpoint(config: Dict[str, Any]) -> str | None:
    ...
    if fallback:
        print(
            f"[train_stage1] Requested resume checkpoint not found: {checkpoint_path}. "
...
def main() -> None:
    ...
    samples = filter_stage1_samples(load_converted_samples(config["dataset_path"]))
    ...
    print(f"Stage 1 adapter saved to {final_adapter_dir}")
